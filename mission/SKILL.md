...
4. **Test gate** — mandatory before marking complete:
   - Run the project's full test suite (check CLAUDE.md, or AGENTS.md if no CLAUDE.md, for the command). All tests must pass.
...
- **Tests are not optional** — every phase must pass the project's test suite before being marked complete. Check CLAUDE.md (or AGENTS.md if no CLAUDE.md) for test commands. If no tests exist for the code you wrote, write them. If the project has a `/scaffold-tests` skill, consider running it for new modules.
