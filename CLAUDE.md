...
├── conductor/SKILL.md      # Autonomous supervisor loop (VISION.md → dispatch skill → oracle gate → loop)
...
- **conductor** is the autonomous tier *above* the other skills. Where you normally invoke spec/mission/scaffold-tests/sherlock by hand one at a time, `/conductor` reads a `VISION.md` north star, picks the next outcome, dispatches the right skill for it, and gates the result on a hard oracle (VISION acceptance criteria + `bun test` + `tsc --strict`) confirmed by a fresh-context verifier — looping unattended until the vision is satisfied. It reuses sherlock's never-pause autonomy and the fresh-context verifier pattern, but is driven by intent rather than code-health scanning. It dispatches the other skills rather than reimplementing them, and like sherlock never pushes.
